...
let files={},lastJob=null;
function url(){return ENGINE_URL}
...
async function api(path,opts={}){const r=await fetch(url()+path,opts);if(!r.ok)throw new Error(await r.text());return r}
async function connect(){try{const r=await api('/health');const j=await r.json();$('connection').textContent='● Connected';$('connection').classList.add('online');add('Connected to The Haven Engine.','system');return j}catch(e){$('connection').textContent='● Connection failed';$('connection').classList.remove('online');add('The Haven Engine is unreachable: '+e.message,'system')}}
$('send').onclick=async()=>{const text=$('prompt').value.trim();if(!text)return;add(text,'user');$('prompt').value='';try{const r=await api('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,files:Object.keys(files)})});const j=await r.json();add(j.reply,'assistant')}catch(e){add('The Haven Engine error: '+e.message,'system')}}
